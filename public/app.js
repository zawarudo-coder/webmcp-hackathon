/* Shared canvas state — the single source of truth for human+agent co-editing */
const collabState = {
  version: 1,
  notes: [],          // {id,text,x,y,owner}
  draws: [],          // {id,path,owner}
...
function listTools() {
  ...
  ul.innerHTML = "";
  const tools = [
    { name: "add_note", desc: "Add a text note to the shared canvas (human or agent)" },
    { name: "move_note", desc: "Move a note to a new position" },
    { name: "list_notes", desc: "List all notes currently on the canvas" },
    { name: "add_agent", desc: "Register a new AI agent persona on the canvas" },
...
if (window.document.modelContext && typeof window.document.modelContext.registerTool === "function") {
  ...
  mc.registerTool({
    name: "add_note",
    description: "Add a text note to the shared canvas. owner should be 'human' or 'agent'.",
    inputSchema: {
      type: "object",
      properties: {
    ...
    execute: async (input) => {
      loadState();
      const note = { id: String(collabState.nextId++), text: input.text, x: input.x, y: input.y, owner: input.owner };
      collabState.notes.push(note);
      saveState();
      log(`[tool:add_note] ${note.owner} added note #${note.id} at (${note.x},${note.y}): "${note.text}"`);
  ...
  mc.registerTool({
    name: "list_notes",
    description: "Return all notes currently on the shared canvas.",
  ...
  mc.registerResource({
    ...
    description: "The full shared canvas state (notes, draws, agents).",
    read: async () => {
      loadState();
  ...
  log("WebMCP tools registered: add_note, list_notes, move_note, add_agent, clear_canvas");
} else {
...
function initHumanControls() {
  document.getElementById("canvas").addEventListener("click", (e) => {
    ...
    if (window.document.modelContext) {
      // prefer the registered tool so the canvas is the canonical state
      window.document.modelContext.callTool("add_note", { text: "💡 idea", x: Math.round(svgP.x), y: Math.round(svgP.y), owner: "human" })
        .then(() => { loadState(); render(); })
        .catch(() => {
          // fallback: write locally
          loadState();
          collabState.notes.push({ id: String(collabState.nextId++), text: "💡 idea", x: Math.round(svgP.x), y: Math.round(svgP.y), owner: "human" });
          saveState(); render();
        });
    } else {
      loadState();
